...
- [x] Node.js LTS installed → `node -v` and `npm -v` work (e.g. v20.x, npm 10.x)  
- [ ] Repo folder opened in Cursor (you’re in **my-prototype**)  
- [ ] (When applicable) `cd prototype && npm install` run successfully — **if EACCES on ~/.npm**, fix cache ownership first (see “If Something Fails” below)  
...
- **EACCES on `~/.npm/_cacache`:** Fix ownership so your user owns the cache, then retry `npm install`:
  ```bash
  # macOS / Linux (run in a terminal; you’ll be prompted for your password)
  sudo chown -R $(whoami) ~/.npm
  cd prototype && npm install
  ```
  **Workaround without sudo:** use a local cache so npm doesn’t touch `~/.npm`:
  ```bash
  cd prototype && npm install --cache .npm-cache
  ```
  (`.npm-cache` is in `.gitignore`.)
